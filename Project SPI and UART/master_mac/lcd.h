...
{	
   // char str_data[4]=0;
	  ...
	  temp_str_data[0] = '0' + (val / 100);              // hundreds
    temp_str_data[1] = '0' + ((val / 10) % 10);        // tens
    temp_str_data[2] = '0' + (val % 10);               // units
   // return str_data;
}
void int_to_string_mac(unsigned int val, unsigned char *temp_str_data)
{
    temp_str_data[0] = '0' + (val / 1000);               // thousands place
    temp_str_data[1] = '0' + ((val / 100) % 10);         // hundreds place
    temp_str_data[2] = '0' + ((val / 10) % 10);          // tens place
    temp_str_data[3] = '0' + (val % 10);                 // units place
                                 // null terminator
}
...
void lcd_clear(void)
{
	...
	msdelay(50);		// Small delay for LCD to process
}
